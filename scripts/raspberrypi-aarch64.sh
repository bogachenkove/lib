...
sudo sh -c "echo \"en_US.UTF-8 UTF-8\" > /etc/locale.gen";sudo locale-gen >> /dev/null;sudo sh -c "echo \"en_US.UTF-8 UTF-8\" > /etc/default/locale"
...
sudo apt install --no-install-recommends --no-install-suggests --yes \
 ...
 encfs cryfs whois default-jre-headless
...
sudo apt install --no-install-recommends --no-install-suggests --yes \
 ...
 fonts-noto-cjk ttf-mscorefonts-installer fonts-font-awesome ranger
echo 'Installing the extra packages.'
sudo apt install --no-install-recommends --no-install-suggests --yes \
 tmux firefox chromium vlc scrot rxvt-unicode speedtest-cli retroarch yt-dlp transmission-cli \
